...
My project also has rate limit of 200req per day so it should be fine as an MVP. Using AI summary does not take API limit away unless rate limited by provider due to too many requests. 


# Tech Stack

Frontend with vanilla JS, HTML and CSS
Backend server with Node.js and express handling OpenWeather API requests + CheckWX requests + Nvidia API setup for AI model
...
I wanted to build something related to aviation. I first thought of only METAR report but then I got the idea of weather app. While building the project, I got the idea of doing AI summary because sometimes I get confused with METAR and ask an AI model for help. I thought why not build one integrated so nobody gotta open 2 tabs for one job :D
...
It also helped me when paragraph separation did not work properly as intended.
...
Most likely, the issue is from checkWX because metar report shows visibility less than 10km. 
